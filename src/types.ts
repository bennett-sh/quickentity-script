...
export interface IProperty extends ISimpleProperty {
  postInit?: boolean;
}
...
export type IExposedEntity =
  | { isArray: true; refersTo: TRef[] }
  | { isArray: false; refersTo: TRef };
...
export type TArrayPatchOperation =
  | { RemoveItemByValue: string }
  | { AddItem: any }
  | { AddItemBefore: [string, string] }
  | { AddItemAfter: [string, string] };

export type IEventTriggers = {
  [key: string]:
    | TRef
    | TRef[]
