...
export class QNEntity {
  ...
  constructor(pathOrData: (IPath & { subType: TSubType }) | IQNEntity) {
  ...
  public addRoot(entityConfig: IRootEntity): Entity {
    if (this.__data.rootEntity)
      console.warn(
        "WARN: multiple root entites are not allowed; overriding previous root entity",
      );
    let { id, name } = entityConfig;
    if (!id) id = "fffffffffffffffe";
    if (!name)
      name = this.__data.subType.replace(/^(\w)/g, (_, m) => m.toUpperCase());
  ...
  public addEntity(entityConfig: ICreateChildEntity): Entity {
    ...
    if (this.__data.entities.hasOwnProperty(id))
      throw new DuplicateEntityIDError(
  ...
  public async save(
    path: string,
    options: Partial<QNSaveOptions> = {},
  ): Promise<void> {
    await writeFileSync(
      path,
      buildJSON(this.build())
        .addIf(options.includeSchema ?? true, {
          $schema:
            "https://raw.githubusercontent.com/atampy25/simple-mod-framework/main/Mod%20Manager/src/lib/entity-schema.json",
        })
