...
PatchEntity.prototype.randomAction = function (actions, options = {}) {
  ...
  const optionEntities = actions.map((option) => {
    const isConditional = scope(option as IPatchConditionalEventTriggers, [
      (x) => x.hasOwnProperty("condition"),
      (x) => x.hasOwnProperty("triggers"),
    ]).allTrue;
    return root.addChild({
      ...getClassPath("RandomSelectorChoice"),
      ...options.action,
      properties: buildJSON(options.action?.properties ?? {})
        .addIf(isConditional, {
          m_Condition: {
      ...
      events: {
        ...
        OnPicked: buildJSON(options.action?.events?.OnPicked ?? {})
          .addIf(
            isConditional,
            (option as IPatchConditionalEventTriggers).triggers,
          )
